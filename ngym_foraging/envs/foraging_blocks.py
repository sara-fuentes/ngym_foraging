...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 14 17:52:13 2024
...
class ForagingBlocks(ngym.TrialEnv):
    """
    ...
    def __init__(self, dt=100, rewards=None, timing=None, probs=None,
                 sigma=1.0, dim_ring=2, blk_dur=200):
        ...
        if probs is None:
            ...
            self.probs = probs
        self.blk_dur = blk_dur
        # Define observations
        name = {'fixation_cue': 0}
        self.observation_space = spaces.Discrete(1, name=name)
        # define action spaces
        name = {'no_action': 0, 'fixation': 1,
                'choice': np.arange(2, dim_ring+2)}
    ...
    def _new_trial(self, **kwargs):
        ...
        # Trial info
        if self.num_tr % self.blk_dur == 0:
            new_probs = self.probs.tolist()
            first_element = new_probs.pop(0)
            new_probs.append(first_element)
            self.probs = np.array(new_probs)
